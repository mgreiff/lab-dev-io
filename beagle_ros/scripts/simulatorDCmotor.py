...
class MotorY(object):
    ...
    def __init__(self):
        self.routine_sub = rospy.Subscriber('LHR_y_routine', String, self.handle_routine_data)
	self.position_pub = rospy.Publisher('LHR_y_pos', Float64, queue_size = 10)
	...
	print '...motor Y complete!'
	self.status_pub.publish('Y')

def main():
    rospy.init_node('MotorY')
    mY = MotorY()
    rospy.spin()
